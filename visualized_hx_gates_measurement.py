...
# 4️⃣ Measurement
qc_measure = QuantumCircuit(number_of_qbits, number_of_qbits)
qc_measure.measure(0, 0)  # Measure the qubit

# Run simulation after measurement
compiled_circuit_measure = transpile(qc_measure, simulator)
result_measure = simulator.run(compiled_circuit_measure).result()
counts_measure = result_measure.get_counts(0)

# Plot measurement results
plot_histogram(counts_measure).savefig('measurement_histogram.png')

print("Bloch sphere images and measurement histogram saved as:")
print("- 'initial_bloch.png' (Initial State |0⟩)")
print("- 'hadamard_bloch.png' (After Hadamard Gate)")
print("- 'x_gate_bloch.png' (After X Gate)")
print("- 'measurement_histogram.png' (Measurement Results)")
